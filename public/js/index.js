...
var app = new Vue({
      ...
      keyboard: {
        ...
        clickNumber: (d) => {
          ...
          if (d == 'delete') {
            ...
            if (hasSymbol) {
              let __last = this.keyboard.money.charAt(
                this.keyboard.money.length - 1,
              );
              if (__last == '-' || __last == '+') {
                this.keyboard.doneName = '完成';
              } else {
  ...
  watch: {
    ...
    'chart.detailList': {
      handler(newList) {
        ...
        if (newList)
          this.initAccountBookDetailList(newList, this.chart.selectTypeId);
      },
    },
    'chart.visibleType'(n) {
      this.myChartInit();
  ...
  computed: {
    ...
    c_accountBookMonthList_GroupByDay() {
      ...
      for (let i = 0, l = this.accountBookMonthList.length; i < l; i++) {
        ...
        groupJson[dateKey].sum[e.tt] =
          Number(groupJson[dateKey].sum[e.tt]) + Number(e.m);
  ...
  methods: {
    ...
    myChartInit() {
      console.log(
        'this.chart.detailList',
        JSON.stringify(this.chart.detailList),
      );
      let source = [];
    ...
    change_payBudgetMonthList_sort_fn(key) {
      let temp = [].concat(
        this.payBudgetMonthList[this.pay_budget_type_is_small_or_big],
      );
      temp.sort((a, b) => {
        return b[key] - a[key];
      });
      this.payBudgetMonthList[this.pay_budget_type_is_small_or_big] = [].concat(
        temp,
      );
    },
    copyDateFn() {
    ...
    showAccountBookMonthSelectFn() {
      ...
      this.datePicker.confirm = () => {
        this.datePicker.show = false;
        this.datePicker.type = 'date';
        this.accountBookMonthList_month =
          this.datePicker.value.format('yyyy-MM');
        this.showAccountBookByMonthFn();
      };
    },
    ...
    showSearchDateSelectFn(time_type_key) {
      ...
      this.datePicker.confirm = () => {
        this.datePicker.show = false;
        this.datePicker.type = 'date';
        this.search.data[time_type_key] =
          this.datePicker.value.format('yyyy-MM-dd');
      };
    },
    resetSearch() {
    ...
    hideIndexGroupWhereInsertPayBudget() {
      ...
      if (this.selectTypePopup.title[0] == '预算') {
        ...
        console.log(
          'hideIndexGroup: ' +
            JSON.stringify(this.selectTypePopup.hideIndexGroup),
        );
      }
    },
    ...
    showPayBudgetMonthListDetailFn(d) {
      ...
      console.log(
        'showPayBudgetMonthListDetailFn:' +
          this.pay_budget_type_is_small_or_big,
      );
      if (this.pay_budget_type_is_small_or_big == 'big') {
    ...
    showAccountBookDetailByTimeFn() {
      this.http('api/accountBook', {
        ...
        if (e.tip == 1) {
          ...
          console.log(
            'smallTypeConfig:' + JSON.stringify(this.chart.smallTypeConfig),
          );
          console.log(
            'bigTypeConfig:' + JSON.stringify(this.chart.bigTypeConfig),
          );
        }
      });
    },
